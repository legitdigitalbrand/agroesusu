...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (isLoading) return (
    <div className="min-h-screen bg-paper flex items-center justify-center"><LoadingState message="Verifying admin access…" /></div>
  );
  if (!me) return (
    ...
    return (
      <div className="min-h-screen bg-paper flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-paper border border-line rounded-2xl p-8 text-center">
          ...
          {bootstrapError && <div className="bg-clay-light rounded-xl p-3 mb-4 text-left"><p className="text-[13px] text-clay">{bootstrapError}</p></div>}
          <button onClick={handleBootstrap} disabled={bootstrapping} className="w-full py-3 bg-ochre text-indigo-deep rounded-xl font-semibold text-[15px] hover:opacity-90 transition disabled:opacity-50">
            {bootstrapping ? "Creating admin access…" : "Become Super Admin (Dev Only)"}
          </button>
          <Link href="/dashboard" className="block mt-3 text-[13px] text-ink-soft hover:text-ink transition">Back to dashboard</Link>
        </div>
      </div>
    );
  }

  const isActive = (href: string) => href === "/dev/dashboard" ? pathname === href : pathname?.startsWith(href);
  ...
  return (
    <div className="min-h-screen bg-paper flex">
      ...
      <aside className="fixed inset-y-0 left-0 z-50 w-64 bg-indigo-deep flex-col hidden md:flex">
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          {adminNav.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition", active ? "bg-paper/10 text-ochre" : "text-white/70 hover:text-white hover:bg-paper/5")}>
                <Icon className="h-5 w-5" strokeWidth={active ? 2.5 : 1.8} /> {item.name}
              </Link>
            );
          })}
        </nav>
        <div className="px-3 py-4 border-t border-white/10">
          <Link href="/" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-paper/5 w-full transition">
            <LogOut className="h-5 w-5" /> Exit console
          </Link>
        </div>
      </aside>
      ...
      <aside className={cn("fixed inset-y-0 left-0 z-50 w-64 bg-indigo-deep flex flex-col md:hidden transition-transform duration-300", drawerOpen ? "translate-x-0" : "-translate-x-full")}>
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          {adminNav.map(item => {
            ...
            const active = isActive(item.href);
            return (
              <Link key={item.href} href={item.href} className={cn("flex items-center gap-3 px-3 py-3 rounded-lg text-sm font-medium transition min-h-[44px]", active ? "bg-paper/10 text-ochre" : "text-white/70 hover:text-white hover:bg-paper/5")}>
                <Icon className="h-5 w-5" strokeWidth={active ? 2.5 : 1.8} /> {item.name}
              </Link>
            );
          })}
        ...
        <div className="px-3 py-4 border-t border-white/10">
          <Link href="/" className="flex items-center gap-3 px-3 py-3 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-paper/5 w-full transition min-h-[44px]">
            <LogOut className="h-5 w-5" /> Exit console
          </Link>
        </div>
      </aside>
      ...
      <div className="flex-1 md:ml-64 flex flex-col min-w-0">
        <header className="sticky top-0 z-30 bg-paper border-b border-line md:hidden flex items-center justify-between px-4 py-3">
          <button onClick={() => setDrawerOpen(true)} className="h-10 w-10 rounded-lg flex items-center justify-center text-ink hover:bg-parchment transition" aria-label="Open menu"><Menu className="h-5 w-5" /></button>
          <div className="flex items-center gap-2"><LogoMark size={24} variant="admin" /><span className="font-display text-sm text-ink">Operations</span></div>
        </header>
