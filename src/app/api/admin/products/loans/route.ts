import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@/lib/supabase/server';
import { createServiceClient } from '@/lib/supabase/service';

async function verifyRole(supabase: ReturnType<typeof createClient>, roleNames: string[]) {
  for (const role of roleNames) {
    const { data } = await supabase.rpc('has_role', { p_role_name: role });
    if (data) return true;
  }
  return false;
}

// POST /api/admin/products/loans — create a new loan product (admin only)
export async function POST(request: NextRequest) {
  try {
    const supabase = createClient();
    const { data: { user }, error: authError } = await supabase.auth.getUser();
    if (authError || !user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });

    const authorized = await verifyRole(supabase, ['super_admin', 'operations']);
    if (!authorized) return NextResponse.json({ error: 'Admin access required' }, { status: 403 });

    const body = await request.json();
    const { product_code, product_name, interest_rate, interest_method, term_months_min, term_months_max, max_multiplier, min_savings_balance, requires_cooperative, origination_fee, processing_fee, description } = body;

    if (!product_code || !product_name || !interest_rate) {
      return NextResponse.json({ error: 'product_code, product_name, and interest_rate are required' }, { status: 400 });
    }

    const serviceClient = createServiceClient();
    const { data: staff } = await supabase.from('staff_users').select('id').eq('auth_id', user.id).maybeSingle();

    const { data: product, error } = await serviceClient
      .from('loan_products')
      .insert({
        product_code,
        product_name,
        interest_rate,
        interest_method: interest_method || 'flat',
        term_months_min: term_months_min || 1,
        term_months_max: term_months_max || 12,
        max_multiplier: max_multiplier || 3,
        min_savings_balance: min_savings_balance || 0,
        requires_cooperative: requires_cooperative || false,
        origination_fee: origination_fee || 0,
        processing_fee: processing_fee || 0,
        description: description || '',
        is_active: true,
        created_by: staff?.id || user.id,
        updated_by: staff?.id || user.id,
      })
      .select()
      .single();

    if (error) return NextResponse.json({ error: `Failed to create product: ${error.message}` }, { status: 500 });

    await serviceClient.from('admin_action_log').insert({
      admin_user_id: staff?.id || user.id,
      admin_role: 'super_admin',
      action: 'create_loan_product',
      action_category: 'product_config',
      entity_type: 'loan_product',
      entity_id: product.id,
      after_state: product,
      result: 'success',
      metadata: { product_code },
    });

    return NextResponse.json({ product }, { status: 201 });
  } catch (error) {
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
