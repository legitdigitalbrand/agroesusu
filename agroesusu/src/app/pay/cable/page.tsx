...
import { ArrowLeftIcon, TvIcon } from '@/components/icons';
...
export default function CablePage() {
  ...
  useEffect(() => {
    fetch('/api/pay/services')
      ...
      .then(data => {
        if (data.services) {
          const cableService = data.services.find((s: any) => s.identifier === 'CABLETV');
          if (cableService) {
            fetch(`/api/pay/services?serviceId=${cableService._id}`)
              .then(res => res.json())
              .then(catData => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/pay/cable', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        setSuccess('Cable TV subscription purchased successfully!');
    ...
    <div className="p-4 lg:p-8 max-w-xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "var(--pot-icon-bg)" }}>
          <TvIcon style={{ width: 20, height: 20, color: "var(--qa-icon-color)" }} />
        </div>
        <div>
          <h1 className="text-xl font-bold" style={{ color: "var(--text-primary)" }}>Cable TV</h1>
          <p className="text-xs" style={{ color: "var(--text-muted)" }}>DStv, GOtv, Startimes subscriptions</p>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label className="block text-sm font-medium mb-2" style={{ color: "var(--text-secondary)" }}>Provider</label>
        ...
        <div>
          <label className="block text-sm font-medium mb-1" style={{ color: "var(--text-secondary)" }}>Smart Card Number</label>
        ...
        <button type="submit" disabled={loading || !cardNumber || !selectedProduct}
          className="w-full py-3.5 rounded-xl font-semibold transition disabled:opacity-50"
          style={{ background: "var(--accent)", color: "var(--accent-contrast)" }}>
          {loading ? 'Processing…' : amount ? `Subscribe · ${formatNaira(Number(amount))}` : 'Select a package'}
        </button>
      </form>
    </div>
  );
